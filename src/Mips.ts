...
export class MipsAssembler extends MipsInstructions {
  ...
  setLabels(rows: String[]): void {
    ...
    rows.forEach((row, index) => {
      if (row.includes(":")) {
        const splitLabel: string[] = row.trim().split(":");
        const commandType = mipsType(splitLabel[1].trim());
        const instruction = this.formatMipsInstruction(
          commandType,
          splitLabel[1]
        );
   ...
   */
  formatMipsInstruction(
    type: string,
    command: String
  ): Instruction["instruction"] {
    //pega cada palavra
    const commandSplit = command
      .trim()
      .split(" ")
      .toString()
      .split(",")
    ...
    let operation: Instruction["instruction"] = {
      opcode: 0,
      rs: 0,
      rt: 0,
      rd: 0,
    ...
    switch (type) {
      case "r":
        operation.opcode =
          this.operationCodes[
            commandSplit[0] as keyof typeof this.operationCodes
          ];
        operation.funct =
          this.functionCodes[
            commandSplit[0] as keyof typeof this.functionCodes
          ];

        if (
          operation.funct === 0 ||
          (operation.funct === 2 && operation.opcode === 0)
        ) {
          operation.rd =
            this.registers[commandSplit[1] as keyof typeof this.registers];
          operation.rt =
            this.registers[commandSplit[2] as keyof typeof this.registers];
        ...
        operation.rt =
          this.registers[commandSplit[2] as keyof typeof this.registers];
        operation.rd = commandSplit[3]
          ? this.registers[commandSplit[3] as keyof typeof this.registers]
          : 0;
        break;
      ...
      case "i":
        ...
        if (operation.opcode !== 15) {
          ...
          operation.jumpInstruction =
            operation.opcode === 4 || operation.opcode === 5 ? true : false;

          //se for uma label pega o endereco
          if (isNaN(operation.immediate)) {
            //pegar o endereco (linha da label - linha da instrucao - 1)
            operation.immediate =
              this._labels.find((label) => label.label === commandSplit[3])
                ?.adress || 0;
          }
        } else {
          operation.rt =
      ...
      case "j":
        ...
        if (isNaN(operation.adress)) {
          operation.adress =
            this._labels.find((label) => label.label === commandSplit[1])
              ?.adress || 0;
        }
        break;
    }
  ...
  get int32BitsArray(): Int32Array {
    ...
    this.readRows.forEach((row, index) => {
      const label = this._labels.filter(
        (label) => label.lineIndex === index
      )[0];
      const instruction = this._instructions.filter(
        (instruction) => instruction.lineIndex === index
      )[0];
      ...
      if (label) {
        ...
        tempArray.push(
          this.convertInstructionBool(instruction, instruction.type)
        );
      }
    });
    ...
    switch (type) {
      ...
      case "i":
        ...
        if (typeI.jumpInstruction) {
          typeI.immediate =
            this._labels.find((label) => label.adress === typeI.immediate)
              ?.lineIndex || 0;
